...
n=length(data_all_ASLO_coords2)
...
for (i in 1:n) {
  ...
  df$airplanes_distance<-df_C$Ind_travelled_distance
  df_loc$Total_emissions_all_flying[i]<-df_C$Total_emissions_all_flying
...
#data_all_ASLO_emissions provides data for individual attendees for all ASM
#df_loc summarizes results for all conferences (summed across attendees)
